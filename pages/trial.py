...
def aggregate_spend(df, consolidated_df):
    ...
    # Convert spend data to DataFrame and calculate total and percentage
    spend_df = pd.DataFrame(spend_data)
    total_spend = spend_df['Spend'].sum()
    spend_df['Total Spend'] = total_spend
...
def main():
    ...
    if uploaded_file is not None:
        ...
        if filter_option == "Spend Variables":
            ...
            st.subheader("Aggregated Spend Data with Total and Percentage Contribution")
            st.write(spend_df)
